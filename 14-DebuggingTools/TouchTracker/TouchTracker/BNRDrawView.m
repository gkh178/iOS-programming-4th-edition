...
@property (strong, nonatomic) IBOutlet UIView *colorSelectView;//指向颜色选择视图
@property (strong, nonatomic) UIColor *selectedColor;//选择的颜色
@end
...
- (void)drawRect:(CGRect)rect {
    ...
    for (BNRLine *line in self.finishedLines) {
        [self.selectedColor setStroke];//用选择的颜色绘制已经画的线
    ...
    //无用代码，用来测试Time Profiler性能的，能测出sin函数耗费大量时间
    float f = 0.0;
...
- (void) strokeLine:(BNRLine *)line {
    ...
    path.lineWidth = line.width;//使用线的宽度属性画线
...
- (void) doubleTap:(UIGestureRecognizer *)gr {
    ...
    self.finishedLines = [[NSMutableArray alloc]init];//导致内存泄露
...
- (void) moveLine:(UIPanGestureRecognizer *)gr {
    ...
    //如果拖动时，菜单可见说明选中了某条线，此刻使菜单不可见并将已选中的线取消
...
- (void) popSelectColorView:(UISwipeGestureRecognizer *)gr {
    NSLog(@"Three fingers swiped");
    [[NSBundle mainBundle]loadNibNamed:@"SelectColorView" owner:self options:nil];//加载SelectColorView视图
    
    CGRect frame = CGRectMake(0, self.window.frame.size.height - 550, self.window.frame.size.width, 50);//视图弹出的位置大小
    self.colorSelectView.frame = frame;
    self.colorSelectView.backgroundColor = [UIColor grayColor];
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    for (UITouch *t in touches) {
        ...
        CGPoint velocity = [self.moveRecognizer velocityInView:self];//获取拖动手势在视图中的矢量速度
        line.width = (velocity.x + velocity.y) / 30;//线的宽度跟速度成正比
        NSLog(@"线的宽度为：%f", line.width);
    }
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    for (UITouch *t in touches) {
        ...
        [self.finishedLines addObject:line];//将当前手指画的线加入finishedLines数组
