...
@interface BNRItemsTableViewController () <UIPopoverControllerDelegate>
//@property (nonatomic, strong) IBOutlet UIView *headerView;//表示在tableView中的headerView对象
@property (nonatomic, strong) UIPopoverController *imagePopover;//指向选中item的缩略图时需要显示的popover对象
@end
...
- (void)viewDidLoad {
    ...
    //创建一个UINib对象，指向BNRItem.xib资源文件
    UINib *nib = [UINib nibWithNibName:@"BNRItemCell" bundle:nil];
    //对BNRItemsTableViewController注册行使用相应的nib文件
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //创建或重用BNRItemCell对象。当对象池中没有BNRItemCell对象时候应该初始化BNRItemCell对象，并将其加到可重用的对象池中；如果有的话直接从对象池中取出
    ...
    cell.actionBlock = ^{
        ...
        //如果设备是Pad，用popover方式弹出所选item的放大图
        if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad) {
            NSString *itemKey = item.itemKey;//获取当前点中的item的key
            ...
            //转换当前cell的bounds矩阵到self.view中的坐标矩阵
            CGRect rect = [self.view convertRect:strongCell.thumbnailView.bounds fromView:strongCell.thumbnailView];
            
            //创建BNRImageViewController对象为image属性赋值
...
- (void)updateTableViewForDynamicTypeSize {
    static NSDictionary *cellHightDictionary;
    
    if (!cellHightDictionary) {
        cellHightDictionary = @{UIContentSizeCategoryExtraSmall : @44,
    ...
    //根据首选字体的大小得到 行高的NSNumber类型包装数
    NSString *userSize = [[UIApplication sharedApplication] preferredContentSizeCategory];
    NSNumber *cellHeight = cellHightDictionary[userSize];
    //设置行高
