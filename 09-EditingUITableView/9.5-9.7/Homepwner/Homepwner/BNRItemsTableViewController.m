...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [[[BNRItemStore sharedStore]allItems]count] + 1;//包含多少个BNRItems就有多少行+1
}
...
//继承自UITableViewDelegate协议的方法，表示修改UITableView对象的某行在删除时的删除确认标题，默认是Delete改为Remove
- (NSString *)tableView:(UITableView *)tableView titleForDeleteConfirmationButtonForRowAtIndexPath:(NSIndexPath *)indexPath {
    return @"Remove";
}


//继承自UITableViewDataSource协议的方法，表示UITableView对象的行的可编辑状态，为NO表示不可编辑
- (BOOL)tableView:(UITableView *)tableView canEditRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    //行为 No more items！不可编辑
    if ([cell.textLabel.text isEqual:@"No more items!"]) {
        return NO;
    }
    
    return YES;//默认行的可编辑状态为YES
}

//继承自UITableViewDataSource协议的方法，表示UITableView对象的行的可移动状态，为NO表示不可移动
- (BOOL)tableView:(UITableView *)tableView canMoveRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    //行为 No more items！不可移动
    if ([cell.textLabel.text isEqual:@"No more items!"]) {
        return NO;
    }
